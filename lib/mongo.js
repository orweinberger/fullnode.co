...
exports.openConnection = function (callback) {
  ...
  var options = {};
  mongodb.connect("mongodb://localhost/" + config.mongo.dbname, options, function (err, db) {
    if (err)
      return callback(err);
...
exports.update = function (collectionName, query, data, callback) {
  var self = this;
  var options = {};
  var _fetch = function () {
    _db.collection(collectionName, options, function (err, collection) {
      if (err)
        return callback(err);
      collection.update(query, data, function (err, data) {
        if (err)
          return callback(err);
        return callback(null, data);
      });
