...
exports.checkDelete = function (date, callback) {
  MongoClient.connect("mongodb://localhost:27017/" + config.mongo.dbname, function (err, db) {
    ...
    sq.findOne({"timestamp": {$lte: date}}, function (err, result) {
      if (err)
        return callback(err);
      if (result) {
        sq.update({"_id": new ObjectId(result._id)}, {$set: {provisioned: 0}}, function (err, res) {
          linode.deleteServer(serverid, function (err) {
            if (err)
              return callback(err)
            else
